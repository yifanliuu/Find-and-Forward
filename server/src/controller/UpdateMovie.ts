...
export async function UpdateMovie(context: Context) {
    ...
    for (var i = 0; i < subtask_list.length; i++) {
        ...
        if (Message != null) {
            ...
            Contrib_invitor.invite_contrib = Contrib_invitor.invite_contrib - min_before_contrib + min_now_contrib;
            Contrib_invitor.total_contrib = Contrib_invitor.work_contrib + Contrib_invitor.invite_contrib;
            Contrib_invitor.reward = Contrib_invitor.work_contrib * Contrib_invitor.work_contrib + Contrib_invitor.invite_contrib;
        ...
        for (var j=0;j<Message_list.length;j++) {
            ...
            invitor_change = invitor_change - min_before_contrib + min_now_contrib;
        }
        
        Contrib_user.work_contrib = Contrib_user.work_contrib + 1;
        Contrib_user.invite_contrib = Contrib_user.invite_contrib + invitor_change;
        Contrib_user.total_contrib = Contrib_user.work_contrib + Contrib_user.invite_contrib;
        Contrib_user.reward = Contrib_user.work_contrib * Contrib_user.work_contrib + Contrib_user.invite_contrib;
